...
namespace Memory
{
    ...
    FixedSizeBufferPool::FixedSizeBufferPool(uint32_t size,
            uint32_t initialCount, uint32_t maxTotalSize) :
            size(size), initialCount(initialCount), maxTotalSize(maxTotalSize), buffers(), borrowedCount(
                    0)
    {
        init();
    }
    ...
    void FixedSizeBufferPool::init()
    {
        ...
        if (initialCount * size > maxTotalSize)
            throw std::invalid_argument("size * initialCount > maxTotalSize");
